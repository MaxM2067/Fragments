...
- **Color Language:**
  ...
  - **Gold (`--color-gold` / Amber Gradient):** Used for **"Daily Minimum"** — mandatory habits. Features floating 3D crystals with sparkles. Integrated into the `HeaderDashboard` as a progress bar that met, causes the dashboard's crystal icon to fill with gold.
...
- `components/`:
  ...
  - `Statistics.tsx`: Detailed blue-accented fragments dashboard and charts.
