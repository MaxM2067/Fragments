  ...
  return (
    <div className="space-y-6 pb-24 text-cozy-text">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-white rounded-cozy p-5 shadow-md shadow-indigo-50/50 border-4 border-indigo-50/20 space-y-4">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-white px-5 py-3 rounded-bubble text-sm outline-none border-4 border-indigo-50/10 focus:border-cozy-indigo/30 font-black"
            />
            <button
              onClick={addCategory}
              className="bg-cozy-indigo text-white px-5 py-3 rounded-bubble active:scale-95 transition-transform shadow-lg shadow-indigo-100 border-b-4 border-indigo-700"
            >
          ...
          <div className="space-y-3">
            {categories.map(cat => (
              <div key={cat.id} className="rounded-bubble bg-indigo-50/30 border-2 border-indigo-50/50 overflow-hidden transition-all duration-300">
                <div className="p-3 flex items-center gap-3">
                  <button
                    onClick={() => setEditingColorId(editingColorId === cat.id ? null : cat.id)}
                    className="relative w-10 h-10 rounded-bubble overflow-hidden shadow-md shrink-0 border-4 border-white transition-transform active:scale-90"
                        ...
                        <div className="flex flex-wrap gap-2">
                          {COZY_PALETTE.map(color => (
                            <button
                              ...
                              className={`w-7 h-7 rounded-bubble border-2 transition-all ${cat.color === color ? 'border-white scale-125 shadow-lg ring-2 ring-cozy-indigo/20' : 'border-transparent scale-100 hover:scale-110'}`}
                              style={{ backgroundColor: color }}
                            />
                          ))}
                          <div className="w-7 h-7 rounded-bubble border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 relative hover:border-cozy-indigo hover:text-cozy-indigo transition-colors">
                            <Plus size={14} />
                            <input
                              type="color"
                              value={cat.color}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {habits.map(habit => (
            <div key={habit.id} className="bg-white p-4 rounded-bubble shadow-md shadow-indigo-50/50 flex items-center justify-between border-4 border-indigo-50/20 group">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div
                  className="w-12 h-12 rounded-bubble flex items-center justify-center text-white shrink-0 shadow-lg shadow-black/5"
              ...
              <div className="flex gap-1 ml-2">
                <button
                  ...
                  className="p-2.5 text-indigo-400 hover:bg-indigo-50 rounded-bubble transition-all active:scale-90"
                >
                  <Edit2 size={18} strokeWidth={3} />
                </button>
                <button
                  onClick={() => onDeleteHabit(habit.id)}
                  className="p-2.5 text-rose-300 hover:text-rose-50 rounded-bubble transition-all active:scale-90"
                >
      ...
      <div className="bg-indigo-100 p-6 rounded-bubble border-4 border-white shadow-xl shadow-indigo-100/50 flex items-start gap-4">
        <div className="w-12 h-12 bg-white rounded-bubble flex items-center justify-center shrink-0 text-indigo-400 shadow-inner">
          <Sparkles size={24} />
        </div>
        <div>
